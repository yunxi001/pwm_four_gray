...
static void Two_gray_sensor_IO_Init(void)
{
    ...
    // 3.结构体参数设置，采用模拟输入模式
    GPIO_InitStruct.GPIO_Pin = Analog_2_PIN | Analog_3_PIN | Analog_1_PIN;
...
static void Adc_Init(void)
{
    ...
    // DMA参数配置
    DMA_InitStruct.DMA_BufferSize = 3;                                       // 读取2个长度的数据
    DMA_InitStruct.DMA_DIR = DMA_DIR_PeripheralSRC;                          // 方向选择外设到内存
    DMA_InitStruct.DMA_M2M = DMA_M2M_Disable;                                // 不开启内存到内存
    DMA_InitStruct.DMA_MemoryBaseAddr = (uint32_t)Analog_data;               // 将存储adc数据的数组地址传入
    ...
    ADC_InitStruct.ADC_NbrOfChannel = 3;                             // ADC通道数
...
{
    float left_norm = (Analog_data[0] - 500) / 1800.0; // 1800 = 2300 - 500
    float right_norm = (Analog_data[1] - 500) / 1800.0;
}

void Two_gray_sensor_Test(void)
{
    printf("Analog_0:%d\t Analog_1:%d\r\n", Analog_data[0], Analog_data[1]);
    // 延时处理
    Delay_ms(500);
}
